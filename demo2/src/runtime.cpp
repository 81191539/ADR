                        ...
                        std::string& error)
{
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (arg == "--cpu-thread-mode" || arg == "--benchmark-thread-mode") {
            if (i + 1 >= argc) {
            ...
            if (arg == "--cpu-thread-mode") {
                if (!parse_cpu_thread_mode(value, exec_config.cpu_thread_mode, error)) {
                    return false;
                }
            } else {
                exec_config.benchmark_mode = true;
                if (!parse_cpu_thread_mode(value, exec_config.benchmark_thread_mode, error)) {
                    return false;
                }
                if (exec_config.benchmark_thread_mode == CpuThreadMode::Auto) {
                    error = "--benchmark-thread-mode must be case or internal.";
                    return false;
                }
            }
            continue;
        ...
        const std::string benchmark_thread_mode_prefix = "--benchmark-thread-mode=";
        ...
        if (arg.rfind(benchmark_thread_mode_prefix, 0) == 0) {
            exec_config.benchmark_mode = true;
            if (!parse_cpu_thread_mode(arg.substr(benchmark_thread_mode_prefix.size()),
                                       exec_config.benchmark_thread_mode,
                                       error)) {
                return false;
            }
            if (exec_config.benchmark_thread_mode == CpuThreadMode::Auto) {
...
void fill_zone_indices(AdsorptionZone& zone, long nx, double h)
{
    zone.i_begin = 0;
    zone.i_end = -1;
    for (long i = 0; i <= nx; ++i) {
...
PreparedCase prepare_case(const Params& p)
{
    ...
    fill_zone_indices(prepared.zone, nx, h);
...
int run_benchmark(const ExecutionConfig& exec_config)
{
    ...
    for (int case_number : exec_config.benchmark_case_numbers) {
        cases.push_back(prepare_case(read_parameter(case_number)));
    }
    ...
    if (exec_config.benchmark_thread_mode == CpuThreadMode::InternalParallel) {
        const PreparedCase& prepared = cases.front();
        SimFields fields;
        fields.resize(prepared.grid.nx, prepared.grid.ny);
        fields.zero_all();
        initialization(fields, prepared.x_ini_posi, prepared.grid);

        long shared_iterations = 0;
        double ct = 0.0;
        bool done = false;

        #pragma omp parallel num_threads(concurrency) shared(fields, shared_iterations, ct, done)
        {
            while (true) {
                #pragma omp single
                {
                    done = omp_get_wtime() >= measure_end;
                    if (!done) {
                        ct = static_cast<double>(shared_iterations + 1) * prepared.dt;
                    }
                }
                #pragma omp barrier
                if (done) {
                ...
                full_step_explicit_in_parallel(fields, prepared.grid, prepared.phys,
                                               prepared.zone, ct, prepared.dt);
        ...
        total_iterations = static_cast<double>(shared_iterations);
    } else {
    ...
    std::cout << "{\"benchmark\":true"
              << ",\"thread_mode\":\"" << cpu_thread_mode_name(exec_config.benchmark_thread_mode) << "\""
                      ...
                      const ExecutionConfig& exec_config)
{
    ...
    const bool use_internal_parallel =
        exec_config.backend == ComputeBackend::Cpu &&
        exec_config.cpu_thread_mode == CpuThreadMode::InternalParallel;

    if (use_internal_parallel) {
        const int internal_threads = compute_cpu_thread_budget(omp_get_num_procs());
        long it = start_iteration;
        long current_it = start_iteration;
        ...
        bool dense_due = false;

        std::cout << "CPU internal parallelism: " << internal_threads
                  << " thread(s) inside case " << case_number << "." << std::endl;

        #pragma omp parallel num_threads(internal_threads) shared(fields, it, current_it, ct, dt, max_it, ns, count, eta_ave, old_eta, eta_ave_dt, last_completed_iteration, last_stability_checked_iteration, stability_abort, unstable_flag, need_check, did_reset, break_loop, loop_done, eta_sum, eta_count, handle_dense, dense_due)
        {
            auto internal_check_stability = [&](long iteration, bool force) {
                #pragma omp single
                {
                    const bool scheduled =
                        config::STABILITY_CHECK_INTERVAL <= 1 ||
                        iteration % config::STABILITY_CHECK_INTERVAL == 0;
                    need_check = (force || scheduled) &&
                                 last_stability_checked_iteration != iteration;
                    unstable_flag = 0;
                    did_reset = false;
                    if (need_check) {
                ...
                if (need_check) {
                    has_unstable_values_in_parallel(fields.cc, grid.nx, grid.ny, unstable_flag);
                }
                #pragma omp barrier

                #pragma omp single
                {
                    if (need_check && unstable_flag) {
                        did_reset = handle_unstable_state(iteration);
                    }
                }
                #pragma omp barrier
                return did_reset;
            ...
            while (true) {
                #pragma omp single
                {
                    loop_done = it > max_it || stability_abort || run_log.converged;
                    break_loop = false;
                    if (!loop_done) {
                        current_it = it;
                        ct += dt;
                ...
                full_step_explicit_in_parallel(fields, grid, phys, zone, ct, dt);

                #pragma omp single
                {
                    std::swap(fields.cc, fields.nc);
                    std::swap(fields.ee, fields.ne);
                    last_completed_iteration = current_it;
                }
                #pragma omp barrier

                if (internal_check_stability(current_it, false)) {
                    #pragma omp single
                    {
                        it = 1;
                    }
                ...
                if (current_it % config::STATS_INTERVAL == 0) {
                    if (internal_check_stability(current_it, true)) {
                        #pragma omp single
                        {
                            it = 1;
                        }
                    ...
                    #pragma omp barrier
                    compute_eta_average_in_parallel(fields.ee, grid, zone, eta_sum, eta_count);

                    #pragma omp single
                    {
                        eta_ave = (eta_count > 0) ? eta_sum / static_cast<double>(eta_count) : 0.0;
                        eta_ave_dt =
                        ...
                        if (run_log.convergence_history.empty()) {
                            ...
                            if (progress - last_progress >= 10.0 ||
                                last.rel_err / rel_err >= 2.0) {
                                should_record = true;
                            }
                        }
                        if (should_record) {
                            run_log.convergence_history.push_back({current_it, ct, eta_ave, rel_err});
                        }
                        ...
                        if (current_milestone > last_milestone && current_milestone <= 100) {
                            ...
                            #pragma omp critical
                            {
                                std::printf("[Case %d][%s/internal] %3d%% | eta=%.4f | err=%.1e | used %.1fs\n",
                        ...
                        if (config::CONVERGENCE_THRESHOLD > 0 &&
                            ...
                            run_log.actual_iterations = current_it;
                            ...
                            #pragma omp critical
                            {
                                ...
                                std::printf("[Case %d][%s/internal] Converged! eta=%.4f | used %.1fs",
                ...
                #pragma omp single
                {
                    handle_dense = dense_dump_enabled &&
                                   (!dense_dump_active ||
                                    dense_next_target < dense_target_offsets.size());
                    dense_due = handle_dense && dense_snapshot_due(current_it);
                }
                #pragma omp barrier

                if (handle_dense) {
                    if (dense_due && internal_check_stability(current_it, true)) {
                        #pragma omp single
                        {
                            it = 1;
                        }
                    ...
                    #pragma omp single
                    {
                        ...
                        if (!break_loop) {
                            if (!dense_dump_active &&
                                ct + sim::EPS_EPOCH >= config::T_DENSE_DUMP_START) {
                                start_dense_window(current_it);
                            }
                            record_due_dense_snapshots(current_it);
                        }
                    }
                    #pragma omp barrier
                    if (break_loop) {
                ...
                if (current_it % config::CHECKPOINT_INTERVAL == 0) {
                    if (internal_check_stability(current_it, true)) {
                        #pragma omp single
                        {
                            it = 1;
                    ...
                    #pragma omp single
                    {
                        ...
                        if (!break_loop) {
                            ...
                            backend->sync_host(fields);
                            save_checkpoint(chkpt_filename.c_str(), chkpt,
                                            fields.cc, fields.ee, nx, p.ny);
                        }
                    }
                    #pragma omp barrier
                    if (break_loop) {
...
int run_cases(const ExecutionConfig& exec_config)
{
    ...
    std::cout << "  Backend: "
              ...
              << std::endl;
    if (exec_config.backend == ComputeBackend::Cpu) {
        std::cout << "  Logical Processors: " << logical_processors << std::endl;
    } else {
    ...
    if (exec_config.backend == ComputeBackend::Cpu &&
        ...
        max_parallel_cases = std::min(static_cast<int>(case_numbers.size()), thread_budget);
        max_parallel_cases = std::max(1, max_parallel_cases);
        ...
        if (exec_config.backend == ComputeBackend::Cpu) {
            std::cout << "CPU internal parallelism: serial case scheduling, budget "
                      << compute_cpu_thread_budget(logical_processors)
                      << " of " << logical_processors << " logical processor(s)."
                      << std::endl;
        }
