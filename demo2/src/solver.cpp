...
void full_step_explicit(SimFields& fields,
                        ...
                        double ct, double dt)
{
    update_eta_and_phi_boundaries(fields.ee, fields.ne, fields.cc,
                                  grid, phys, zone, dt);

    oscillatory(phys.alpha, phys.Sc, fields.ff, fields.yy, ct, grid.ny);

    calc_phi_with_advection(fields.cc, fields.nc, fields.yy, fields.ff,
                            dt, grid.nx, grid.ny, grid.h,
                            phys.Pe, phys.Pe2);
}

void full_step_explicit_in_parallel(SimFields& fields,
                                    const GridInfo& grid,
                                    const PhysicsParams& phys,
                                    const AdsorptionZone& zone,
                                    double ct, double dt)
{
    update_eta_and_phi_boundaries_in_parallel(fields.ee, fields.ne, fields.cc,
                                              grid, phys, zone, dt);

    oscillatory_in_parallel(phys.alpha, phys.Sc, fields.ff, fields.yy, ct, grid.ny);

    calc_phi_with_advection_in_parallel(fields.cc, fields.nc, fields.yy, fields.ff,
                                        dt, grid.nx, grid.ny, grid.h,
                                        phys.Pe, phys.Pe2);
}
...
//-----------------------------------------------------------------------------
// Fused eta update and concentration boundary conditions.
//-----------------------------------------------------------------------------
                                   ...
                                   double dt)
{
    double* left_ghost = cc.physical_row_data(-1);
    double* left = cc.physical_row_data(0);
    const double* right = cc.physical_row_data(grid.nx);
    double* right_ghost = cc.physical_row_data(grid.nx + 1);
    for (long j = cc.y_lower_bound(); j <= cc.y_upper_bound(); ++j) {
        left_ghost[j] = phys.c0;
        left[j] = phys.c0;
        right_ghost[j] = right[j];
    }

    const double K0_inv = 1.0 / phys.K0;
    const double h_Da = grid.h * phys.Da;
    const double coeff = phys.eps * phys.Da;
    const double* ee_data = ee.physical_data();
    double* ne_data = ne.physical_data();

    long ads_begin = zone.i_begin;
    long ads_end = zone.i_end;
    if (ads_begin < 0) {
        ads_begin = 0;
    }
    if (ads_end > grid.nx) {
    ...
    #pragma omp parallel
    {
        ...
        for (long i = 0; i <= grid.nx; ++i) {
            double* cc_i = cc.physical_row_data(i);
            cc_i[grid.ny + 1] = cc_i[grid.ny];
            cc_i[-1] = cc_i[0];
            ne_data[i] = 0.0;
        }

        #pragma omp for schedule(static)
        for (long i = ads_begin; i <= ads_end; ++i) {
            double* cc_i = cc.physical_row_data(i);
            const double eta = ee_data[i];
            const double adsorption = cc_i[0] * (1.0 - eta) - eta * K0_inv;
            ne_data[i] = eta + dt * coeff * adsorption;
            cc_i[-1] = cc_i[0] - h_Da * adsorption;
        }
    }
}

void update_eta_and_phi_boundaries_in_parallel(Field1D& ee, Field1D& ne,
                                               Field2D& cc,
                                               const GridInfo& grid,
                                               const PhysicsParams& phys,
                                               const AdsorptionZone& zone,
...
void oscillatory_in_parallel(double alpha, double Sc,
                             Field1D& ff, const Field1D& yy,
                             double ct, long ny)
{
    const double ca = std::cos(alpha);
...
//-----------------------------------------------------------------------------
// Fused branchless upwind advection and explicit concentration update.
//-----------------------------------------------------------------------------
                             ...
                             double Pe, double Pe2)
{
    ...
    for (long i = 0; i <= nx; ++i) {
        ...
        for (long j = 0; j <= ny; ++j) {
            const double y = yy_data[j];
            const double du = Pe * y * (1.0 - y) + Pe2 * ff_data[j];
            const double back_diff = (cc_i[j] - cc_im[j]) * h_inv;
            const double fwd_diff = (cc_ip[j] - cc_i[j]) * h_inv;
            const double pos = 0.5 * (du + std::fabs(du));
            const double neg = 0.5 * (du - std::fabs(du));
            const double adv = pos * back_diff + neg * fwd_diff;
            const double lap =
                ((cc_ip[j] - 2.0 * cc_i[j] + cc_im[j]) +
                 (cc_i[j + 1] - 2.0 * cc_i[j] + cc_i[j - 1])) * h2_inv;
            nc_i[j] = cc_i[j] + dt * (lap - adv);
        }
    }
}

void calc_phi_with_advection_in_parallel(const Field2D& cc, Field2D& nc,
                                         const Field1D& yy, const Field1D& ff,
                                         double dt, long nx, long ny, double h,
                                         double Pe, double Pe2)
{
...
void has_unstable_values_in_parallel(const Field2D& field, long nx, long ny,
                                     int& found_unstable)
{
    constexpr double negative_tolerance = -1e-12;
    constexpr double max_reasonable_magnitude = 1e12;

    #pragma omp for schedule(static) reduction(|:found_unstable)
    for (long i = 0; i <= nx; ++i) {
        const double* field_i = field.physical_row_data(i);
        for (long j = 0; j <= ny; ++j) {
            const double value = field_i[j];
...
void compute_eta_average_in_parallel(const Field1D& eta,
                                     const GridInfo& grid,
                                     const AdsorptionZone& zone,
                                     double& eta_sum,
                                     long& eta_count)
{
    ...
    #pragma omp for schedule(static) reduction(+:eta_sum, eta_count)
    for (long i = 0; i <= grid.nx; ++i) {
        const double x = grid.xleft + static_cast<double>(i) * grid.h;
        if (x > zone.xpo_l && x < zone.xpo_r) {
            eta_sum += eta_data[i];
