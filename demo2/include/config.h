...
namespace config {
     ...
     * - 0 = auto: one case uses internal parallelism, multiple cases use case parallelism
     * - 1 = case parallelism: one OpenMP worker per concurrent case
     * - 2 = internal parallelism: one case at a time, persistent OpenMP region inside each case
     */
    constexpr int CPU_THREAD_MODE = 1;
    /** @} */
