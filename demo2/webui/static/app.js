...
const warmupThreadModeEl = document.getElementById("warmup-thread-mode");
...
function formatThreadMode(value) {
  if (value === 1 || value === "case") return "case";
  if (value === 2 || value === "internal") return "internal";
  return "auto";
}
...
function renderWarmup(status = {}) {
  ...
  const cpuThreadMode = status.cpuThreadMode ?? state.env?.cpuThreadMode ?? 0;
  ...
  if (warmupThreadModeEl) warmupThreadModeEl.textContent = formatThreadMode(cpuThreadMode);
  if (warmupCaseCountEl) warmupCaseCountEl.textContent = String(count || 0);
  if (warmupBestEl) {
    warmupBestEl.textContent = best
      ? `${best.threadMode || "case"} / ${best.concurrency}，预计 ${formatEstimatedMakespan(best.estimatedMakespan)}`
      : "-";
  }
  ...
  if (applyWarmupBtnEl) {
    applyWarmupBtnEl.disabled = isRunning ||
      !state.selectedWarmupConcurrency ||
      !state.selectedWarmupMode;
  }
  ...
  if (warmupResultsBodyEl) {
    ...
    if (!results.length) {
      ...
      const bestMakespan = Math.min(
        ...results
          .map((row) => Number(row.estimatedMakespan))
          .filter((value) => Number.isFinite(value) && value > 0)
      );
      results.forEach((result) => {
        const row = document.createElement("tr");
        const concurrency = Number(result.concurrency);
        const threadMode = result.threadMode || "case";
        row.className = concurrency === state.selectedWarmupConcurrency &&
          threadMode === state.selectedWarmupMode ? "warmup-row-selected" : "";
        row.addEventListener("click", () => {
          state.selectedWarmupConcurrency = concurrency;
          state.selectedWarmupMode = threadMode;
          renderWarmup(state.warmupStatus || {});
        });
        const makespan = Number(result.estimatedMakespan);
        const relative = Number.isFinite(bestMakespan) && bestMakespan > 0 && Number.isFinite(makespan) && makespan > 0
          ? `${((bestMakespan / makespan) * 100).toFixed(1)}%`
          : "-";
        [
          threadMode,
          concurrency,
          ...
          formatEstimatedMakespan(result.estimatedMakespan),
          relative,
...
async function handleApplyWarmup() {
  try {
    if (!state.selectedWarmupConcurrency || !state.selectedWarmupMode) {
      throw new Error("请先在结果表中选择一个模式和并发数。");
    }
    await api("/api/warmup/apply", {
      method: "POST",
      body: JSON.stringify({
        concurrency: state.selectedWarmupConcurrency,
        threadMode: state.selectedWarmupMode,
      }),
    });
