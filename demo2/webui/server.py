...
CPU_THREAD_MODE_PATTERN = re.compile(r"(constexpr\s+int\s+CPU_THREAD_MODE\s*=\s*)(\d+)(\s*;)")
...
def read_cpu_thread_mode_config() -> int:
    if not CONFIG_PATH.exists():
        return 0
    match = CPU_THREAD_MODE_PATTERN.search(read_text_file(CONFIG_PATH))
    return int(match.group(2)) if match else 0


def write_cpu_thread_mode_config(value: int) -> None:
    if value not in (0, 1, 2):
        raise ValueError("CPU_THREAD_MODE must be 0, 1, or 2.")
    content = read_text_file(CONFIG_PATH)
    if not CPU_THREAD_MODE_PATTERN.search(content):
        raise ValueError("Could not find CPU_THREAD_MODE in config.h.")
    updated = CPU_THREAD_MODE_PATTERN.sub(lambda m: f"{m.group(1)}{value}{m.group(3)}", content, count=1)
    write_text_file(CONFIG_PATH, updated)


def parse_cpu_thread_mode_label(value: str) -> int:
    normalized = value.strip().lower()
    if normalized in ("case", "case-parallel", "1"):
        return 1
    if normalized in ("internal", "internal-parallel", "2"):
        return 2
    if normalized in ("auto", "0"):
        return 0
    raise ValueError("Thread mode must be case or internal.")
...
def environment_snapshot() -> dict[str, Any]:
    ...
    current_cpu_thread_mode = read_cpu_thread_mode_config()
    ...
    return {
        ...
        "cpuThreadMode": current_cpu_thread_mode,
        "checks": checks,
    }
...
class WarmupState:
    ...
    def snapshot(self) -> dict[str, Any]:
        return {
            ...
            "cpuThreadMode": read_cpu_thread_mode_config(),
        }
...
def estimate_warmup_makespan(result: dict[str, Any], case_count: int) -> dict[str, Any]:
    ...
    thread_mode = str(result.get("threadMode", "case"))
    ...
    if thread_mode == "internal":
        waves = normalized_case_count
        rate = total_rate
    else:
...
def run_warmup_benchmarks(case_ids: list[int],
    ...
    with WARMUP_STATE.lock:
        ...
        WARMUP_STATE.append_log(
            f"[info] Starting warmup with {len(candidates)} candidate(s), {len(modes)} mode(s), and {len(case_ids)} sample case(s)."
        )
        ...
        for mode in modes:
            for candidate in candidates:
                with WARMUP_STATE.lock:
                    if WARMUP_STATE.stop_requested:
                        break
                    WARMUP_STATE.current = f"{mode}:{candidate}"
                    WARMUP_STATE.append_log(f"[info] Benchmarking {mode} mode with concurrency {candidate}.")

                solver_args = [
                    "--benchmark-thread-mode", mode,
                ...
                if exit_code != 0:
                    raise RuntimeError(f"Benchmark {mode}:{candidate} failed.")
                if payload is None:
                    raise RuntimeError(f"Benchmark {mode}:{candidate} did not return JSON.")

                result = {
                    "threadMode": str(payload.get("thread_mode", mode)),
...
def parse_warmup_modes(raw_modes: Any) -> list[str]:
    if isinstance(raw_modes, list):
        tokens = [str(item).strip().lower() for item in raw_modes]
    else:
        raw = str(raw_modes or "").strip()
        tokens = re.split(r"[\s,]+", raw) if raw else ["case", "internal"]
    modes: list[str] = []
    for token in tokens:
        if token in ("case", "case-parallel", "1") and "case" not in modes:
            modes.append("case")
        elif token in ("internal", "internal-parallel", "2") and "internal" not in modes:
            modes.append("internal")
    if not modes:
        raise ValueError("Warmup modes must include case or internal.")
    return modes
...
def launch_warmup_task(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    modes = parse_warmup_modes(payload.get("modes", ["case", "internal"]))
    ...
    worker = threading.Thread(
        ...
        args=(case_ids, candidates, modes, seconds, warmup_seconds),
        daemon=True,
    )
    worker.start()
    return {
...
def apply_warmup_concurrency(value: int, thread_mode: str) -> dict[str, Any]:
    logical_processors = os.cpu_count() or 1
    if value < 0 or value > logical_processors:
        raise ValueError(f"Concurrency must be between 0 and {logical_processors}.")
    mode_value = parse_cpu_thread_mode_label(thread_mode)
    if mode_value == 0:
        raise ValueError("Warmup apply requires case or internal mode.")
    with WARMUP_STATE.lock:
        if WARMUP_STATE.status == "running":
            raise RuntimeError("A warmup task is still running.")
    write_num_threads_config(value)
    write_cpu_thread_mode_config(mode_value)
    return {
        "applied": True,
        "numThreads": value,
        "cpuThreadMode": mode_value,
        "threadMode": "case" if mode_value == 1 else "internal",
        "config": str(CONFIG_PATH),
    }
    ...
    def do_POST(self) -> None:  # noqa: N802
        ...
        try:
            ...
            if path == "/api/warmup/apply":
                ...
                json_response(
                    self,
                    apply_warmup_concurrency(
                        int(payload.get("concurrency")),
                        str(payload.get("threadMode", "")),
                    ),
                )
                return
            error_response(self, "Unknown endpoint.", status=404)
