%% plot_centerline_cx_v1.m
% Plot centerline concentration profiles from dense concentration fields.
% The centerline is the geometric symmetry line y = yright / 2.
...
[min_y_distance, y_idx] = min(abs(y_vec - y_center));
grid_tol = max(10 * eps(max(1, max(abs(y_vec)))), 1e-12);
use_direct_grid_point = min_y_distance <= grid_tol;

profiles = nan(numel(x_vec), snapshot_count);
reference_cc = [];

fprintf('\nExtracting centerline profiles at y = %.12g (%d snapshots)...\n', ...
    y_center, snapshot_count);
...
for r = 1:snapshot_count
    ...
    if use_direct_grid_point
        profiles(:, r) = cc_data(:, y_idx);
    else
        profiles(:, r) = interp1(y_vec, cc_data.', y_center, 'linear').';
    end
...
%% ======================== Plot field and centerline profiles ========================
...
clim(ax_field, [0, 1]);
hold(ax_field, 'on');
plot(ax_field, [x_plot(1), x_plot(end)], [y_center, y_center], ...
     'k-', 'LineWidth', 2.4);
plot(ax_field, [x_plot(1), x_plot(end)], [y_center, y_center], ...
     'w-', 'LineWidth', 1.2);
hold(ax_field, 'off');
...
ylabel(ax_profile, '{\itc}* at centerline', 'Interpreter', 'tex');
legend(ax_profile, 'Location', 'eastoutside', 'Interpreter', 'tex', 'Box', 'off');
title(ax_profile, sprintf('Centerline profiles at {\\ity} = %.6g', y_center), ...
...
if save_output
    if isempty(output_name)
        output_name = sprintf('centerline_cx_case%d_Pe1_%g_Pe2_%g_snapshots%d-%d.%s', ...
                              case_number, Pe, Pe2, ...
    ...
    fprintf('\nCenterline profile figure saved: %s\n', output_path);
end
