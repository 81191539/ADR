...
function renderWarmup(status = {}) {
  ...
  if (warmupResultsBodyEl) {
    ...
    if (!results.length) {
      ...
      results.forEach((item) => {
        const row = document.createElement("tr");
        row.className = item.concurrency === state.selectedWarmupConcurrency ? "warmup-row-selected" : "";
  ...
  if (startWarmupBtnEl) startWarmupBtnEl.disabled = running || !(state.env && state.env.canRun);
  if (stopWarmupBtnEl) stopWarmupBtnEl.disabled = !running;
  if (applyWarmupBtnEl) applyWarmupBtnEl.disabled = running || !state.selectedWarmupConcurrency;
...
async function handleApplyWarmup() {
  const concurrency = state.selectedWarmupConcurrency || state.warmupStatus?.best?.concurrency;
  if (!concurrency) {
    throw new Error("请先选择一行预热结果。");
  }
  ...
  setWarmupError(`已应用 NUM_THREADS = ${concurrency}。下一次保存并编译运行会生效。`);
  await refreshEnv();
  await refreshWarmupStatus();
}
