...
__global__ void stability_check_kernel(DeviceField2D field, long nx, long ny, int* has_unstable)
{
    const long i = blockIdx.x * blockDim.x + threadIdx.x;
    const long j = blockIdx.y * blockDim.y + threadIdx.y;
    if (i > nx || j > ny) {
    ...
    const double value = at_const(field, i, j);
    if (!isfinite(value) || value < -1e-12 || fabs(value) > 1e12) {
        atomicExch(has_unstable, 1);
    }
}
...
public:
                            ...
                            double ct, double dt) override
    {
        ...
        std::swap(device_state_.cc, device_state_.nc);
        std::swap(device_state_.ee, device_state_.ne);
    }

    bool has_unstable_values(SimFields& fields, const GridInfo& grid) override
    {
        ensure_allocated(fields);
        CUDA_CHECK(cudaMemset(d_has_unstable_, 0, sizeof(int)));
        ...
        stability_check_kernel<<<blocks, threads>>>(
            device_state_.cc, grid.nx, grid.ny, d_has_unstable_);
        CUDA_CHECK(cudaGetLastError());
                               ...
                               const AdsorptionZone& zone) override
    {
        ...
        eta_average_kernel<<<blocks, threads>>>(
            device_state_.ee, grid.xleft, grid.h,
...
private:
    ...
    void ensure_allocated(const SimFields& fields)
    {
        ...
        CUDA_CHECK(cudaMalloc(reinterpret_cast<void**>(&d_has_unstable_), sizeof(int)));
    ...
    void release()
    {
        ...
        if (d_has_unstable_ != nullptr) {
            cudaFree(d_has_unstable_);
            d_has_unstable_ = nullptr;
        }
