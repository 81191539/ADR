...
\title{Inertial particle trapping in a counter-rotating vortex pair with unequal  strength}

\author{Zilong Zhao\aff{1}, Zhigang Zuo\aff{2} \and Zhongdong Qian\aff{1}}

\affiliation{\aff{1}Department, Institution, City, Country
\aff{2}Department, Institution, City, Country}

\corresau{Zhongdong Qian, zdqian@whu.edu.cn}
...
\section{Governing equations for the flow and particles}
\label{sec:2}
We describe the dynamics of the carrier flow and suspended particles in the Eulerian--Lagrangian framework. The volume fraction of the particle $\Phi_p$ is sufficiently small ($\Phi_p \le 1 \times 10^{-6}$) and the fluid Reynolds number $Re$ is very high, which allows us to neglect the back-reaction of the particle (corresponds to the one--way coupled regime) and the viscous effect on the carrier flow.

In the present study, our primary focus is on studying the motion of particles in an unequal strength counter-rotating vortex pair flow, which is produced by a system of two counter-rotating vortices, denoted as vortex V1 and V2. The circulation of vortex V1 and V2 are $\Gamma_1(> 0)$ and $\Gamma_2(< 0)$, respectively. The circulation ratio $\gamma = \Gamma_2/ \Gamma_1$ satisfies $-1 < \gamma < 0$. According to the Biot-Savart law, these two vortices revolve around a point denoted as $I$ with an angular speed $\Omega = (\Gamma_1+\Gamma_2)/2\pi b^2$, as illustrated in Figure \ref{fig:1} (a). The separation distance $b$ between two vortex centers remains constant. 

The separation distance $b$, the period of rotation $T=1/ \Omega$, and their ratio $U=d/T$ are defined as the characteristic length, time, and velocity, respectively, to non-dimensionalise the system. We choose a reference frame rotating around the center point $I$ with the non-dimensional angular speed of the vortex pair. In this co-rotating frame, the flow is steady, and the stationary vortices are centred at $\boldsymbol{X}_{c1} = (\gamma/(1+\gamma))$ and $\boldsymbol{X}_{c2} = (1/(1+\gamma))$ , as shown in Figure \ref{fig:1} (b). In the non-dimensional form, the flow field of the counter-rotating vortex pair is given by $\boldsymbol{u}(\boldsymbol{X},\gamma) = (\partial \Psi/ \partial y, -\partial \Psi/ \partial x)$, where $\Psi$ is the stream function in the co-rotating frame. 
\begin{equation}
    \Psi(\boldsymbol{X} = \phi(\boldsymbol{X}-\boldsymbol{X}_{c1}))+\gamma \phi(\boldsymbol{X}-\boldsymbol{X}_{c2}))+2\left| \boldsymbol{X}\right|^2,
    \label{eq:1}
\end{equation}
where $ \phi(\boldsymbol{X}) = -\ln\left| \boldsymbol{X} \right|/(1+\gamma)$ , $\boldsymbol{X}$ is the position vector in the co-rotating frame. 

\begin{figure}[htpb]
  \centerline{\includegraphics[scale=0.95]{Figure-1.pdf}}% Images in 100% size
...
\section{Trapping dynamics of weakly inertial particles}
\label{sec:3}

\section{Moderately inertial particles trapping and leakage}
\label{sec:4}
